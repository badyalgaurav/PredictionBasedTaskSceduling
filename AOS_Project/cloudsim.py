...
def init_task_generators(scenario):
    ...
    scenario.printSep()
    print
    "- Task Generators created"
    print
    "%s\t:\t %s" % ("Jobs to be generated", str(totalJobs))
...
def print_initial_data(scenario):
    ...
    "- Initial data:"
    print
    "%s\t:\t %s" % ("Execution mode", str(scenario.mode))
    print
    "%s\t:\t %s" % ("Random seed", str(scenario.seed))
    print
    "%s\t:\t %s" % ("Scheduling algorithm", str(scenario.algoName))
    print
    "%s\t:\t %d%%" % ("Acceptable waste", scenario.acceptableWaste)
    print
    "%s\t:\t %s" % ("Started workers", str(scenario.initial_machines))
    print
    "%s\t:\t %s" % ("Simulation time", str(scenario.sim_time))
...
def print_result(scenario):
    ...
    "- Simulation results:"
    print
    "%s\t:\t %d (%.2f%%)" % ("Completed jobs", completedJobs, (float(completedJobs) / totalJobs) * 100)
    print
    "%s\t:\t %.2fs" % ("Total execution time", now())
    print
    "%s\t:\t %.2fs" % ("Total CPU time used", cpuTime)
    print
    "%s\t:\t %.2fs" % ("Total CPU time paid", paidTime)
    print
    "%s\t:\t %.2fs" % ("Total unused paid time", wastedTime)
    print
    "-- %s\t:\t %.2fs (%.2f%%)" % ("Swap, idle and startup:", wastedSwStartup, (wastedSwStartup / wastedTime) * 100)
    print
    "-- %s\t:\t %.2fs (%.2f%%)" % ("Unused partial hour:", wastedPart, (wastedPart / wastedTime) * 100)
    print
    "%s\t:\t %.2f%%" % ("Percentage of waste", (wastedTime / paidTime) * 100)
    print
    "%s\t:\t $%.2f" % ("Total cost", totalCost)
    print
    "%s\t:\t %.2fs" % ("Average job response time", jobRTs.mean())
    print
    "%s\t:\t %.2fs" % ("Job response time std deviation", math.sqrt(jobRTs.var()))
